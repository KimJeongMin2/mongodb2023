...
guestRouterRouter.get('/reservation_history/:guestId', async (req, res) => {
  ...
  try {
    ...
    const reservations = await Reservation.find(filter)
      .populate({ path: 'lodging', select: ['name', 'address', 'type', 'checkIn', 'checkOut']})
      .sort({ checkIn: -1 })
      .exec();
...
guestRouterRouter.post('/reviews/:reservationId', async (req, res) => {
  ...
  try {
    ...
    if (reservation.memberId.toString() !== memberId || reservation.lodging.toString() !== lodgingId) {
      return res.status(400).json({ error: '올바른 회원 또는 숙소를 선택해주세요.' });
    }
